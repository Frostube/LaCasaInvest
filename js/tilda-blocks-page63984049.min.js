...
function t1093__processGlobalObj() {
  ...
  if (typeof window.t1093_isMobile === 'undefined') {
    window.t1093_isMobile = /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator
      .userAgent) || ('ontouchend' in document && navigator.userAgent.indexOf('AppleWebKit') !== -1)
  }
...
function t1093__setPointerEventsToZeroBlock(rec) {
  ...
  var hasBG = abFilterBG !== 'none' || artBoardStyle.backgroundColor !== 'rgba(0, 0, 0, 0)' || artBoardStyle
...
function t1093__showOnClick(e) {
  ...
  if (lastOpenedPopupHook) {
    ...
    var shouldClosePopup = lastOpenedPopup && lastOpenedPopup.getAttribute('data-popup-close-on-open-another') ===
      'true';
    if (shouldClosePopup) {
...
function t1093__closeOnLink(e) {
  ...
  if (!popupLink) return;
  var exceptionSelectors = ['.t978__tm-link', '.t966__tm-link', '.t794__tm-link', 'a[href="#closepopup"]',
...
function t1093__showCurrentPopup(popup) {
  ...
  var isLazy = allRecords ? allRecords.getAttribute('data-tilda-lazy') === 'yes' : !1;
...
function t1093__pauseAllVideo(popup) {
  if (!popup) return;
  var videoElems = Array.prototype.slice.call(popup.querySelectorAll(
...
function t786__alignButtons_init(recid) {
  ...
  try {
    ...
    window.addEventListener('resize', t_throttle(function() {
      if (typeof window.noAdaptive !== 'undefined' && window.noAdaptive && window.isMobile) {
        return
      }
      t786__alignButtons(recid)
    }));
    var wrapperBlock = rec.querySelector('.t786');
    if (wrapperBlock) {
...
function t786__showMore(recid) {
  ...
  if (!showMoreButton.querySelector('td').textContent) {
    showMoreButton.querySelector('td').textContent = t786__dict()
  }
  ...
  showMoreButton.addEventListener('click', function() {
    ...
    displayChangedEvent.initEvent('displayChanged', !0, !1);
    rec.dispatchEvent(displayChangedEvent);
    var addBtnEvent = document.createEvent('HTMLEvents');
    addBtnEvent.initEvent('twishlist_addbtn', !0, !1);
...
function t786__alignButtons(recid) {
  var rec = document.getElementById('rec' + recid);
  if (!rec) return;
  var wrappers = rec.querySelectorAll('.t786__textwrapper');
  var maxHeight = 0;
  var container = rec.querySelector('.t786__container');
  if (container) {
  ...
  var mobileOneRow = window.innerWidth <= 960 && rec.querySelector('.t786__container_mobile-flex');
  var mobileTwoItemsInRow = window.innerWidth <= 480 && rec.querySelector('.t786 .mobile-two-columns');
  if (mobileView) {
    itemsInRow = 1
  }
  if (tableView) {
  ...
  Array.prototype.forEach.call(wrappers, function(wrapper) {
    ...
    if (itemsInRow === 1) {
      ...
      Array.prototype.forEach.call(wrappersInRow, function(card) {
...
function t786__updateLazyLoad(recid) {
  ...
  if (scrollContainer && currentMode !== 'edit' && currentMode !== 'preview') {
    scrollContainer.addEventListener('scroll', t_throttle(function() {
      if (window.lazy === 'y' || document.getElementById('allrecords').getAttribute('data-tilda-lazy') ===
        'yes') {
        t_onFuncLoad('t_lazyload_update', function() {
          t_lazyload_update()
        })
...
function t786_initPopup(recid) {
  ...
  Array.prototype.forEach.call(openerLink, function(link) {
    ...
    function showPopupHandler(event) {
      event.preventDefault();
      if (event.target.classList.contains('t1002__addBtn') || event.target.parentNode.classList.contains(
          't1002__addBtn')) {
        return
      }
...
function t786_checkUrl(recid) {
  var currentUrl = window.location.href;
  var tprodIndex = (currentUrl.indexOf('#!/tproduct/') + 1 || currentUrl.indexOf('%23!/tproduct/') + 1 || currentUrl
    .indexOf('#%21%2Ftproduct%2F') + 1 || currentUrl.indexOf('#!%2Ftproduct%2F') + 1 || currentUrl.indexOf(
      '%23%21%2Ftproduct%2F') + 1) - 1;
  if (tprodIndex !== -1) {
...
function t702_lockScroll() {
  var documentBody = document.body;
  if (!documentBody.classList.contains('t-body_scroll-locked')) {
    var bodyScrollTop = typeof window.pageYOffset !== 'undefined' ? window.pageYOffset : (document.documentElement ||
      documentBody.parentNode || documentBody).scrollTop;
...
function t702_showPopup(recId) {
  ...
  if (/iPhone|iPad|iPod/i.test(navigator.userAgent) && !window.MSStream && window.isiOSVersion && window.isiOSVersion[
    0] === 11) {
    setTimeout(function() {
      t702_lockScroll()
    }, 500)
...
function t270_scrollToEl(hash, offset) {
  ...
  var canSmoothScroll = window.CSS && window.CSS.supports('scroll-behavior', 'smooth') && 'scrollBehavior' in document
